...
public class Exercise30_isOrdered
{
    ...
    public class BST<Key extends Comparable<Key>, Value>
    {
        ...
        public boolean isOrder(Node x, Key min, Key max)
        {
            ...
            if (min != min(x).key || max != max(x)) return false;
            if (!isOrder(x.left, min, max(x.left))) return false;
            if (!isOrder(x.right, min(x.right).key, max)) return false;
            return true;
